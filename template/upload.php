<?php
function upload($image, $dir = "../images")
{
        list($width,$height,$type) = getimagesize($image["tmp_name"]);
        $new_height = round($height*$width/$width);
        $emp_img = imagecreatetruecolor($width, $height);
    switch ($type) {
        case IMAGETYPE_JPEG:
        ...
        imagecopyresampled($emp_img, $new_image, 0, 0, 0, 0, $width, $height, $width, $height);
        $date = date("YmdHis"). substr(explode(".", (microtime(true) . ""))[1], 0, 3);
    switch ($type) {
        case IMAGETYPE_JPEG:
            imagejpeg($emp_img, $dir."/"."KCME".$date.".jpg");
            break;
        case IMAGETYPE_GIF:
            $bgcolor = imagecolorallocatealpha($new_image, 0, 0, 0, 127);
            imagefill($emp_img, 0, 0, $bgcolor);
            imagecolortransparent($emp_img, $bgcolor);
            imagegif($emp_img, $dir."/"."KCME".$date.".gif");
            break;
        case IMAGETYPE_PNG:
            imagepng($emp_img, $dir."/"."KCME".$date.".png");
            break;
    }
